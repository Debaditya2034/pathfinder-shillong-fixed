...
function Driver() {
  ...
  useEffect(() => {
    ...
    const fetchBookingsAndAnalytics = async () => {
      try {
        ...
        // Fetch Analytics
        const analyticsData = await getDriverAnalytics(user.uid);
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          // Refresh analytics if a booking just completed (simple naive check by length or status)
          // For now, only initial load unless we add logic here.
          // To update analytics in real-time we'd need another listener or re-fetch on status change.
          // Let's re-fetch analytics if any booking is 'completed' in the current snapshot 
          // that wasn't before? Or simpler: just rely on initial load for now as per "dashboard analytics" request.
          // But to be cool, let's re-fetch analytics if we detect a completion.
          const hasNewCompletion = bookingsData.some(b => b.status === 'completed' && lastSeenStatusesRef.current.get(b.id) !== 'completed');
          if (hasNewCompletion) {
            getDriverAnalytics(user.uid).then(setAnalytics);
          }
        }, (error) => {
  ...
  const handleBookingUpdates = (bookingsData) => {
    ...
    bookingsData.forEach(booking => {
      ...
      // Only show notification if status changed and we've seen this booking before
      // Also check that we don't already have a dialog open
      if (previousStatus && previousStatus !== currentStatus && !notificationDialog) {
