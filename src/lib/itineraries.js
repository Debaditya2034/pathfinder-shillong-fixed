 ...
 */
export async function createItinerary(userId, places, specialMessage = null, vehicleType = 'Sedan') {
  ...
  try {
    const itineraryRef = await addDoc(collection(db, 'itineraries'), {
      ...
      vehicleType,
      ...calculateTripFare(places, vehicleType),
 ...
 */
export async function updateItinerary(itineraryId, userId, places, specialMessage = null, vehicleType = 'Sedan') {
  if (!places || places.length === 0) {
    throw new Error('Itinerary must have at least one place');
  }
